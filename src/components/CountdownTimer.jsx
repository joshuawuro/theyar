...
function CountdownTimer() {
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      const gmtNow = new Date(now.toUTCString().slice(0, -4)); // Get GMT+0 time
      const target = new Date(gmtNow);
      target.setUTCHours(9, 0, 0, 0); // 9:00 AM GMT+0

      // If past 9:00 AM, set it for the next day
      if (gmtNow > target) target.setUTCDate(target.getUTCDate() + 1);
      ...
      // Check if it's between 9:00 - 10:00 AM GMT+0
      const hour = gmtNow.getUTCHours();
      setIsPrayerTime(hour === 9); // between 9:00 and 9:59
    };
